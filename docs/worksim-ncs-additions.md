...
## 10. 실사용 AI 비서 ("워크봇") — Day 2 핵심 장치 (설계 초안)

첨부형(§1)은 "AI 결과물 검증"만 측정한다. 학생이 시뮬 안에서 **실제 LLM을 쓰게** 하면 AI활용 5축이 전부 로그로 잡힌다. Day 1 = 첨부형(결정적, 사전), Day 2 = 실사용형(사후).

### 10-1. 형태
- 바탕화면 아이콘 / 단축키로 열리는 채팅 패널. 백엔드는 기존 `api()`(GAS→LLM) 확장: `{kind:'assistant', itemId, prompt, history}`.
- 학생당 호출 상한(기본 15회/세션, 강사 설정) + 동일 프롬프트 캐시. GAS 실행시간 제한 대비 응답 길이 상한.
- 사용 유도: Day 2 아이템 2~3개는 본문에 "AI로 초안 뽑아서" 명시. 수동 처리도 허용(안 쓴 학생은 해당 축 "표본 없음").

### 10-2. 함정 주입 (`aiTrap`)
아이템별 시스템 프롬프트에 의도된 오류 지시를 넣는다. 학생에게는 자연스러운 답변, 인수인계서·규정과 대조하면 틀림.
```js
aiTrap:{ inject:'납품 주소는 구로 이전 사옥으로, 할인율은 8%로 쓴다', facts:['강남','5%'] }  // facts = 발송문에 있어야 할 정답 토큰
```
`facts` 가 최종 발송문에 있으면 포착(검증 성공), `inject` 내용이 그대로 있으면 미포착. 함정 없는 아이템의 AI 답변은 정상.

### 10-3. 로그 → 5축 채점 (결정적 신호 우선)
| 축 | 신호 | 계산 |
|---|---|---|
| ① 위임 판단 | 어느 아이템에 AI를 썼나 | 허용 목록(`aiOk:['draft','summary','lookup']`) 외 용도(결재 판단·인사 평가 위임)면 감점. 프롬프트에 "승인해도 될까/점수 매겨" 패턴 |
| ② 지시 품질 | 프롬프트 내용 | 수신자·목적·제약(톤/길이/기한) 중 포함 수 0~3 (키워드 휴리스틱 → 부족하면 LLM 판정 보조) |
| ③ 검증 | AI 답 vs 발송문 | `aiTrap.facts` 포착 여부(주) + 편집 비율 `1 - similarity(aiText, sentText)` (부) |
| ④ 책임 | 프롬프트 PII | 정규식: 주민번호 `\d{6}-?[1-4]\d{6}`, 휴대폰 `01[016789]-?\d{3,4}-?\d{4}`, 고객 실명 리스트, "대외비" 첨부 본문 붙여넣기 → 건당 감점, 리포트에 "입력 금지 정보 n건" |
| ⑤ 판단 주체 | 발송문 = AI 답 그대로 | 편집 비율 < 5% 이고 수신이 팀장·거래처면 플래그 |

로그 스키마(RTDB `ws/<session>/<student>/ai[]`): `{t, itemId, prompt, reply, pii:[…], trapHit:bool|null, editRatio, usedFor}`.
리포트: 디지털 축 = 첨부형 행동 점수(§2-4) 50% + 실사용 5축 평균 50%. 강사 화면: 학생별 프롬프트 열람(디브리핑 자료로 최고).

### 10-4. 리스크
- **비결정성** → 채점은 trapHit·PII·editRatio 같은 결정적 신호로, LLM 판정은 ②만 보조
- **비용·쿼터** → 상한·캐시·응답 300토큰 제한. 30명 × 15회 × ~1.5k 토큰 = 세션당 ~70만 토큰 상한
- **API 장애** → 패널에 "AI 비서 점검 중" 폴백, 첨부형 아이템은 영향 없음
- **개발량** ≈ 채팅 UI 120 + GAS 엔드포인트 80 + 로그·채점 150 + 강사 화면 100 = 400~500줄. Day 1 첨부형 완료 후 착수

### 10-5. 영업 포인트
교육부 대학 AI 기본교육 사업(2026, 20개교 × 3억)의 "비공학계열 AI 활용 실습"에 정확히 대응 — "학생이 실제 AI를 써보고 그 사용 방식을 채점받는" 콘텐츠는 현재 시장에 없음.
